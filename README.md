...
- 이상 감지 시 MS Teams or Slack을 선택적으로 전달할 수 있도록 구성
<br>
...
Grafana에서 지원되지 않는 기능을 처리하기 위해 만든 Python 서버로 Grafana Webhook을 수신하여 SNS로 전달
...
### [02-kube-prometheus-stack](./02-kube-prometheus-stack)

kube-prometheus-stack 기반 Helm 차트 (수집·저장·시각화·알람 규칙을 모두 포함)

- **멀티 클러스터**: 1st는 Grafana+Prometheus 풀셋, 2nd는 Prometheus만 배포 후 원격 datasource로 연결
- **설정 코드화**: Alert Rules·Datasources·Dashboards 등을 ConfigMap/Secret으로 관리 (Grafana UI 의존 없음)
